...
covid_cases_filtered<-covid_cases%>%
  filter(location == c("Canada", "United States", "Italy", "Russia", "Australia", "Egypt","South Africa", "India"))
...
covid_tests_filtered%>% ggplot(aes(x = Date, y = `Cumulative total`)) +
  geom_line(aes(color = Entity), se = TRUE) + labs(title = "Total Tests in Canada US and Italy over our Time Period")
...
covid_response_gatherings_filtered <- covid_response %>% 
  filter(CountryCode == c("CAN", "USA", "ITA","RUS", "AUS", "EGY", "ZAF", "IND"), `C1_School closing` >= 0, `C4_Restrictions on gatherings` >= 0)%>%
...
lifeexpect%>%
  filter(region %in% c("Canada", "United States", "Italy", "Russia", "Australia", "Egypt","South Africa", "India"))%>%
  ggplot() + geom_bar(mapping = aes(x = region, y = `total population`, fill = region, color = region), stat = "Identity")+ylim(0,100)+ labs(title = "Average life expectancy in the chosen countries")
...
deaths_vs_lifeexpect%>%
  ...
  ggplot()+geom_line(mapping = aes(x = date, y = total_deaths, color = region)) + labs(title = "Total Covid deaths by date in Canada, United States, and Italy") + facet_wrap(~region)+ylim(0,150000)

deaths_vs_lifeexpect%>%
  filter(region %in% c("Australia", "Russia", "Egypt"))%>%
  ggplot()+geom_line(mapping = aes(x = date, y = total_deaths, color = region)) + labs(title = "Total Covid deaths by date in Australia, Russia, and Egypt") + facet_wrap(~region)+ylim(0,150000)
...
population_vs_cases%>%
  ...
  ggplot(mapping = aes(x = date, y = `Percentage_Cases_Population`)) + geom_line(color = "blue", position = "dodge") + ylim(0,3)+
